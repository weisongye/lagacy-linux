...
void csi_m(int currcons)
{
	...
	for (i = 0; i <= npar; i++)
		...
		default:
			...
			else /* Background color */ if ((par[i] >= 40)
							&& (par[i] <= 48))
...
:				    );
		...
		case ESsetterm:	/* Setterm functions. */
			...
			if (c == 'S') {
				def_attr = attr;
				video_erase_char =
				    (video_erase_char & 0x0ff) | (def_attr <<
								  8);
			} else if (c == 'L') ;	/*linewrap on */
			else if (c == 'l') ;	/*linewrap off */
			break;
		case ESsetgraph:
			state = ESnormal;
			if (c == '0')

...
void tty_init(void)
{
	...
	for (i = 0; i < QUEUES; i++)
		tty_queues[i] = (struct tty_queue) {
		0, 0, 0, 0, ""};
	rs_queues[0] = (struct tty_queue) {
	0x3f8, 0, 0, 0, ""};
	rs_queues[1] = (struct tty_queue) {
	0x3f8, 0, 0, 0, ""};
	rs_queues[3] = (struct tty_queue) {
	0x2f8, 0, 0, 0, ""};
	rs_queues[4] = (struct tty_queue) {
	0x2f8, 0, 0, 0, ""};
	for (i = 0; i < 256; i++) {
		...
		con_table[i] = (struct tty_struct) {
			{
				ICRNL,	/* change incoming CR to NL */
				    ...
				    0, IXON | ISIG | ICANON | ECHO | ECHOCTL | ECHOKE, 0,	/* console termio */
			INIT_C_CC}, 0,	/* initial pgrp */
			    0,	/* initial session */
			    0,	/* initial stopped */
		con_write,
			    con_queues + 0 + i * 3,
			    con_queues + 1 + i * 3, con_queues + 2 + i * 3};
	}
	for (i = 0; i < NR_SERIALS; i++) {
		...
		B2400 | CS8,
			    ...
			    rs_write,
			    rs_queues + 0 + i * 3,
			    rs_queues + 1 + i * 3, rs_queues + 2 + i * 3};
	}
	for (i = 0; i < NR_PTYS; i++) {
		...
		B9600 | CS8,
			    ...
			    mpty_write,
			    mpty_queues + 0 + i * 3,
			    mpty_queues + 1 + i * 3, mpty_queues + 2 + i * 3};
		...
		B9600 | CS8,
			    ...
			    spty_write,
			    spty_queues + 0 + i * 3,
			    spty_queues + 1 + i * 3, spty_queues + 2 + i * 3};
	}
	rs_init();
